...
let integerLiteral (x: integerLiteral) =
    ...
    System.Int64.Parse(literalDigits digits)
    * (if suffix.IsSome then -1L else 1L),
...
let realLiteral (x: realLiteral) =
    ...
    System.Double.Parse(string)
    * (if suffix.IsSome then -1.0 else 1.0),
    Range.create x.start x.stop

let rec quantifiedType (QuantifiedType (t, _)) = Ast.Type.QuantifiedType(ty t, [], [])
...
and enclosedType (x: enclosedType) =
    ...
    | Nominal (name, args) ->
        ...
        | Some (_, (_, _, x, xs, _, r)) ->
            ...
            Ast.Type.create
                (Ast.Type.Application((Ast.Type.create (Ast.Type.Named ident) identRange.start identRange.stop), args))
                identRange.start
                r.stop
    ...
    | Variable (_, name, args) ->
        ...
        | Some (_, (_, _, x, xs, _, r)) ->
            ...
            Ast.Type.create (Ast.Type.Application((Ast.Type.create (Ast.Type.Variable ident) identStart identStop), args)) identStart r.stop
...
let rec pattern (x: pattern) : Ast.Pattern.Pattern =
    ...
    | Assignment (kw, _, id, _, _, pat) ->
        ...
        Ast.Pattern.create (Ast.Pattern.Assignment(ident id, astPat)) kw.start astPat.range.stop
    | Variant (pat, ts) ->
        ...
        if ts.Length = 0 then
            ...
            |> List.map (fun (_, x) -> x)
            |> List.map ty
            |> List.fold (fun acc x -> Ast.Pattern.create (Ast.Pattern.Variant(x, acc)) astPat.range.start x.range.stop) astPat

and enclosedPattern (x: enclosedPattern) =
    match x with
    | enclosedPattern.Ident id -> Ast.Pattern.create (Ast.Pattern.Identifier(ident id)) id.start id.stop
    ...
    | enclosedPattern.Character (l, c, r) -> Ast.Pattern.create (Ast.Pattern.Literal(Ast.Character(escapedCharacter c))) l.start r.stop
    | enclosedPattern.Boolean x -> Ast.Pattern.create (Ast.Pattern.Literal(Ast.Boolean(x.inner))) x.start x.stop
        ...
        |> List.fold
            (fun acc (_, t) ->
                ...
                Ast.Expression.create
                    ...
                    acc.range.start
                    (match astT with
                     | Ast.Type.QuantifiedType (t, _, _) -> t.range.stop))
            (subExprU e)
    ...
    if updates.Length = 0 then
        ...
        |> List.fold
            (fun acc (_, items) ->
                ...
                Ast.Expression.create (Ast.Expression.RecordUpdate(acc, astItems)) acc.range.start range.stop)
            (subExprA e)

and recordBody ((l, _, x, xs, _, r): recordBody) : (Ast.Ident * Ast.Expression.Expression) list * Range =
    let rest = xs |> List.map (fun (_, x) -> fieldInit x)
    ...
    if es.Length = 0 then
        ...
        |> List.fold
            (fun acc (_, expr) ->
                ...
                Ast.Expression.create (Ast.Expression.Application(acc, astExpr)) acc.range.start astExpr.range.stop)
            (subExprC e)
    ...
    if es.Length = 0 then
        ...
        |> List.fold
            (fun acc (_, expr) ->
                ...
                Ast.Expression.create (Ast.Expression.Application(astExpr, acc)) acc.range.start astExpr.range.stop)
            (atom e)
...
and atom (e: atom) =
    ...
    | Ident x ->
        ...
        Ast.Expression.create (Ast.Expression.Identifier ident) range.start range.stop
    | Parens (l, _, x, xs, _, r) ->
        if xs.Length = 0 then
            expr x
        else
            let items = expr x :: (xs |> List.map (fun (_, x) -> expr x))
            Ast.Expression.create (Ast.Expression.TupleConstructor items) l.start r.stop
    | Unit (l, _, r) -> Ast.Expression.create (Ast.Expression.TupleConstructor []) l.start r.stop
    | Record x ->
        let (items, range) = recordBody x
        Ast.Expression.create (Ast.Expression.RecordConstructor items) range.start range.stop
    | EmptyRecord (l, _, r) -> Ast.Expression.create (Ast.Expression.RecordConstructor []) l.start r.stop
    | List (l, _, x, xs, _, r) ->
        let rec createAstList list =
            match list with
            | [] -> Ast.Expression.create (Ast.Expression.RecordConstructor []) l.start r.stop
            | x :: xs ->
                Ast.Expression.create
                    (Ast.Expression.TupleConstructor [ expr x
                                                       createAstList xs ])
                    l.start
                    r.stop

        createAstList (x :: (xs |> List.map (fun (_, x) -> x)))
    | EmptyList (l, _, r) -> Ast.Expression.create (Ast.Expression.RecordConstructor []) l.start r.stop
    | Block (l, _, x, xs, _, r) ->
        let items =
            x :: (xs |> List.map (fun (_, x) -> x))
            |> List.map expr

        Ast.Expression.create (Ast.Expression.Block items) l.start r.stop
    | Extern (l, _, s, _, e, _, r) ->
        let (name, _) = stringLiteral s
        Ast.Expression.create (Ast.Expression.External(name, expr e)) l.start r.stop
    | String x ->
        let (string, range) = stringLiteral x
        Ast.Expression.create (Ast.Expression.Literal(Ast.String string)) range.start range.stop
    | Integer x ->
        let (value, range) = integerLiteral x
        Ast.Expression.create (Ast.Expression.Literal(Ast.Integer value)) range.start range.stop
    | Real x ->
        let (value, range) = realLiteral x
        Ast.Expression.create (Ast.Expression.Literal(Ast.Real value)) range.start range.stop
    | Character (l, c, r) -> Ast.Expression.create (Ast.Expression.Literal(Ast.Character(escapedCharacter c))) l.start r.stop
    | Boolean x -> Ast.Expression.create (Ast.Expression.Literal(Ast.Boolean x.inner)) x.start x.stop
